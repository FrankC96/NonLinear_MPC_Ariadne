...
class Controller:
    ...
    def __post_init__(self):
        ...
        self.max_input = np.max(self.input_bounds)
        self.min_input = np.min(self.input_bounds)
    ...
    def objective(self, X: np.array, x_ref: np.array):
        ...
        state_cost = np.sum(
            [
                ((X_state[i] - x_ref[i]) @ self.Q) @ (X_state[i] - x_ref[i]).T
    ...
    def constraints(self, X: np.array, x0: np.array, x_ref: np.array):
        ...
        if self.term_constr:
            ceq.extend(X[-1, : self.n_states] - x_ref)
        return ceq
        ...
        for input in range(self.n_inputs):
            for state in range(self.n_states):
                for idx in range(self.n_pred):
                    ...
                    ineq.extend([self.max_input - u[idx, input]])
                    ineq.extend([x[idx, input] - self.min_input])
        return ineq

    def optimize(self, x0: np.array, xref: np.array):
        optimizer_dict = {"x": np.array, "u": np.array, "flag": False}

        X_guess = np.full([self.n_pred, self.n_states + self.n_inputs], np.nan)
        for i in range(self.n_states + self.n_inputs):
            if i < self.n_states:
                X_guess[:, i] = np.linspace(-10, 10, self.n_pred)
            else:
                X_guess[:, i] = np.zeros(self.n_pred)
        ...
        if self.verbose:
            print(
                f"Optimization {optimizer_dict['flag']} with message -> {result.message}."
            )
            print(f"~~~Optimizer cost {result.fun}")
